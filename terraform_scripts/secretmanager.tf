  ...
  secret_string = jsonencode(
    {
      ...
      "uri": join("", ["mongodb://", aws_docdb_cluster.sample.endpoint, ":27017/?replicaSet=rs0&readPreference=secondaryPreferred"])
    }
  )
}
...
resource "aws_secretsmanager_secret_rotation" "mdw-documentdb-app-user" {
  secret_id           = aws_secretsmanager_secret.sample-documentdb-app-user.id
...
resource "aws_lambda_function" "docdb-rotate-lambda" {
  depends_on = [data.archive_file.docdb_lambda_function]
...
resource "aws_lambda_function" "app-user-docdb-rotate-lambda" {
  depends_on = [data.archive_file.app-user-docdb_lambda_function]
...
resource "aws_iam_policy_attachment" "sample-secret-rotation-policy-attachment" {
  ...
  roles = [aws_iam_role.sample-lambda-dbpassword-rotate-role.name]
}
