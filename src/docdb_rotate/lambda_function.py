...
def create_secret(service_client, arn, token):
    logger.info("inside create secret")
    logger.info(service_client)
    logger.info(arn)
    logger.info(token)
...
def set_secret(service_client, arn, token):
    ...
    logger.info("secret token pending:")
    logger.info(pending_dict)
    ...
    logger.info("secret token current:")
    logger.info(pending_dict)

    if not conn:
        # If both current and pending do not work, try previous
        try:
            prev_dict = get_secret_dict(service_client, arn, "AWSPREVIOUS")
            conn = get_connection(prev_dict)
            logger.info("secret token Prev:")
            logger.info(pending_dict)
...
def get_connection(secret_dict):
    ...
    try:
        client = MongoClient(host=secret_dict['host'], port=port, connectTimeoutMS=5000, serverSelectionTimeoutMS=5000, ssl=ssl, ssl_ca_certs='rds-combined-ca-bundle.pem')
        db = client[dbname]
...
def get_secret_dict(service_client, arn, stage, token=None):
    logger.info(f'arn is {arn} and stage is {stage} and token is {token}')
    ...
    logger.info(f'secret dict is {secret_dict}')
